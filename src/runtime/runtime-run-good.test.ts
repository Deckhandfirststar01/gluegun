...
test('runs a command', async t => {
  const r = new Runtime()
  r.plugin(`${__dirname}/../fixtures/good-plugins/threepack`)
...
test('runs a nested command', async t => {
  const r = new Runtime()
  r.plugin(`${__dirname}/../fixtures/good-plugins/nested`)
...
test('runs a command with no name prop', async t => {
  const r = new Runtime()
  r.plugin(`${__dirname}/../fixtures/good-plugins/missing-name`)
